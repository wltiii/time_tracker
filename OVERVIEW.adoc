...
. The domain layer contains the business logic for the application, including the todo item aggregate root. The aggregate root processes the command or query and updates the state of the todo item domain object as necessary.
...
. The infrastructure layer contains the code that interacts with external systems such as databases, file systems, and APIs. It receives the request from the domain layer and performs the necessary CRUD operation on the todo item.
